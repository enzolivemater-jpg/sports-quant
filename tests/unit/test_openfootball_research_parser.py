...
SAMPLE = """= English Premier League 2024/25
...
def test_parser_inherits_time_and_year_state() -> None:
    module = _load_module()

    rows = module.parse_openfootball(SAMPLE, timezone_name="Europe/London", expected_matches=4)
...
def test_missing_halftime_score_remains_missing() -> None:
    module = _load_module()

    rows = module.parse_openfootball(SAMPLE, timezone_name="Europe/London")

    assert rows[3]["home_ht_goals"] is None
    assert rows[3]["away_ht_goals"] is None
...
def test_expected_match_count_is_enforced() -> None:
    ...
    with pytest.raises(ValueError, match="Expected 5 matches, parsed 4"):
        module.parse_openfootball(SAMPLE, timezone_name="Europe/London", expected_matches=5)
